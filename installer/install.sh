...
#!/bin/bash
  ...
  ################################################ 
  ### Vesta Installer V1.0 By Joe Mattos         #
  ################################################
  ...
  ### to be removed
    myVesta_SYSTEM_REPO_CMD="sudo apt-get --quiet --assume-yes"
    
  ### System Related Variables
    myV_System_OS="$(grep "^ID=" /etc/os-release | cut -d'=' -f2 | tr -d '"' | tr '[:upper:]' '[:lower:]')"
    myV_System_Arch="$(dpkg --print-architecture)"
    myV_System_Release="$(grep "^VERSION_ID=" /etc/os-release | cut -d'=' -f2 | tr -d '"')"
    myV_System_CodeName="$(grep "^VERSION_CODENAME=" /etc/os-release | cut -d'=' -f2 | tr -d '"')"
    
    myV_System_Disk="$(df -H / | awk '$3 ~ /[0-9]+/ { print $4 }' | tr -d 'G')"
    myV_System_Memory="$(cat /proc/meminfo | awk '/MemTotal/ { printf $2 / (1024*1024)}')"
    myV_System_HostName="${HOSTNAME}"

  ###
    myVesta_DIR="/usr/local/vesta"  
    myVesta_DIR_BIN="${myVesta_DIR}/bin"
    myVesta_DIR_DATA="${myVesta_DIR}/data"
    myVesta_DIR_CONFIG="${myVesta_DIR}/conf"
    myVesta_DIR_TMP="$(mktemp -d)"
    myVesta_DIR_INSTALL="install/${myV_System_OS}/${myV_System_Release}"

  ###
    myVesta_ADMIN_USERNAME="admin"
    myVesta_ADMIN_PASSWORD="123456"
    myVesta_ADMIN_EMAIL="sizzlinhost@gmail.com"
    myVesta_ADMIN_DOMAIN="j52m.com"
    myVesta_ADMIN_NS_1="ns1.${myVesta_ADMIN_DOMAIN}"
    myVesta_ADMIN_NS_2="ns2.${myVesta_ADMIN_DOMAIN}"
    myVesta_ADMIN_HOSTNAME="myvesta.${myVesta_ADMIN_DOMAIN}"

  ###
    myVesta_INSTALL_HOST="c.myvestacp.com"
    myVesta_INSTALL_REPO="apt.myvestacp.com"
    myVesta_INSTALL_REPO_KEY="https://${myVesta_INSTALL_REPO}/deb_signing.key"
    
    myVesta_INSTALL_PORT="8083"

  ###
    myVesta_FORCE="0" # Y=0 N=1
    myVesta_INTERACTIVE="0" # Y=0 N=1
  ...
  ################################################
    ...
    ### 
    CLEAN_EXIT() {
      [[ -e "${myVesta_DIR_TMP}" ]] && rm -rf ${myVesta_DIR_TMP}
        exit
    }
    
  ### 
    CHECK_PACKAGES_CONFLICTS_DEPENDENCIES_TEST() {
      local RUNIT
      local PACKAGE
      local PACKAGE_FILE="$(mktemp -p ${myVesta_DIR_TMP} 2>/dev/null)"
      local DEPENDENCIES="curl wget gnupg2"
      local MISSING_PACKAGES=""
      local CONFLICTS="exim4 exim4-base exim4-config mysql-server mariadb-server mariadb-client mariadb-common  apache2 apache2-utils nginx vesta"
      local CONFLICTING_PACKAGES=""
        
        ### Grab Packages  
        dpkg --get-selections | grep -v 'deinstall' > ${PACKAGE_FILE}
          [[ ! -s "${PACKAGE_FILE}" ]] && errorMSG "Unable to load Installed Packages."  

        ### Sort Packages
        for PACKAGE in ${DEPENDENCIES[@]} ${CONFLICTS[@]}; do
          ### Remove Conflicting Packages
          if [[ ! -z "$(grep "^${PACKAGE}" ${PACKAGE_FILE})" && ${CONFLICTS[*]} =~ (^|[[:space:]])"${PACKAGE}"($|[[:space:]]) ]]; then
              if ASK_YES_NO "Would you like to remove package ${PACKAGE}"; then
                echo -n " - Removing ${PACKAGE}"
               
                  if [[ "${PACKAGE}" = "mariadb-common" ]]; then
                    local getSource="mariadb-server-$(dpkg -s mariadb-common | grep -i '^Source' | cut -d'-' -f2 )"
                    echo ${getSource} ${getSource}/postrm_remove_databases boolean true | sudo debconf-set-selections
                  fi

                    RUNIT="$(packageCMD "--purge remove ${PACKAGE/apache2/apache2*}"  "N")"
                      [[ "$?" -ne "0" ]] && echo -e "\033[0;31m     [FAILED: ${RUNIT}]]\e[0m" && CONFLICTING_PACKAGES+="${PACKAGE} " || echo -e "\033[0;32m     [REMOVED]\e[0m"
              else
                 echo -e " - Removing ${PACKAGE} \033[0;31m     [FAILED: By Users Input]\e[0m"
                CONFLICTING_PACKAGES+="${PACKAGE} "  
              fi
          ### Install Required Packages  
          elif [[ -z "$(grep "^${PACKAGE}" ${PACKAGE_FILE})" && ${DEPENDENCIES[*]} =~ (^|[[:space:]])"${PACKAGE}"($|[[:space:]]) ]]; then
            if ASK_YES_NO "Would you like to install ${PACKAGE}"; then
                echo -n " - Installing ${PACKAGE}"
                
                RUNIT="$(packageCMD "install ${PACKAGE}" "N")"
                 [[ "$?" -ne "0" ]] && echo -e "\033[0;31m     [FAILED: ${RUNIT}]\e[0m" && MISSING_PACKAGES+="${PACKAGE} " || echo -e "\033[0;32m     [INSTALLED]\e[0m"
            else
                 echo -e " - Installing ${PACKAGE} \033[0;31m     [FAILED: By Users Input]\e[0m"
                 MISSING_PACKAGES+="${PACKAGE} "
            fi  
          fi
        done
        
          #### IF NOT EMPTY DO SOME SHIT
          [[ ! -z "${MISSING_PACKAGES}" ]] && errorMSG "The following package(s) are Required for installation ${MISSING_PACKAGES[*]}please install them."
          [[ ! -z "${CONFLICTING_PACKAGES}" ]] && errorMSG "The following package(s) are already installed ${CONFLICTING_PACKAGES[*]}please remove them."

    }
    
    CHECK_PACKAGES_CONFLICTS_DEPENDENCIES() {
      local RUNIT
      local PACKAGE
      local PACKAGE_FILE="$(mktemp -p ${myVesta_DIR_TMP} 2>/dev/null)"
      local DEPENDENCIES="curl wget gnupg2"
      local MISSING_PACKAGES=""
      local CONFLICTS="exim4 exim4-base exim4-config mariadb-server mariadb-client mariadb-common apache2 apache2-utils nginx vesta mysql-server"
      local CONFLICTING_PACKAGES=""
        ...
        for PACKAGE in ${DEPENDENCIES[@]} ${CONFLICTS[@]}; do
          ...
          if [[ ! -z "${MISSING_PACKAGES}"  || ! -z "${CONFLICTING_PACKAGES}" ]]; then
 
          echo -e "\n Error: Package Conflicts\n"
          echo -e " The following package(s) need to be installed,\n    - ${MISSING_PACKAGES-"None"}\n\n The following package(s) need to be removed,\n   - ${CONFLICTING_PACKAGES-"None"}\n"
            if ASK_YES_NO "Would you like to complete this operation"; then
              echo "yes"
              
              
              REMOVEIT="$(packageCMD "--purge remove ${CONFLICTING_PACKAGES/apache2/apache2*}"  "N")"
                [[ "$?" -ne "0" ]] && errorMSG "${REMOVEIT} "||  echo -e "\033[0;32m     [REMOVED]\e[0m"  && CONFLICTING_PACKAGES="" 
              
              INSTALLIT="$(packageCMD "install ${MISSING_PACKAGES}" "N")"
                 [[ "$?" -ne "0" ]] && errorMSG "${INSTALLIT}" ||  echo -e "\033[0;32m     [INSTALLED]\e[0m" && MISSING_PACKAGES="" 
             
            else
              errorMSG " Failed By Users Input"
            fi  
          fi
  ...
  ### 
    packageCMD() {
      ...
      [[ -z "${CMD}" ]] && errorMSG "Invalid use of Function: packageCMD Line: ${BASH_LINENO[0]}"
      
        RUNIT="$(sudo apt-get --quiet --assume-yes ${CMD} 2>&1)"
          if [[ "$?" -ne "0" ]]; then
            [[ "${RETURN}" = "Y" ]] && errorMSG "APT-GET Failure: ${RUNIT#*E: }" "${BASH_LINENO[0]}" || echo "APT-GET Failure: ${RUNIT#*E: }" && return 1
          fi
    }
    
  ###
    ASK_YES_NO () {
      if [[ ${myVesta_FORCE} -eq "0" ]]; then
        return 0
      else  
        while true; do
  ...
  if [ -z "${BYPASS}" ]; then
    clear
    echo " - System and Installer Requirements Checks"
    echo -e "OS: ${myV_System_OS}\nRelease: ${myV_System_Release}\nDiskSpace ${myV_System_Disk}GB\nMemory: ${myV_System_Memory}GB\nHostname: ${myV_System_HostName}" >> /root/file.txt
    
    ### Checking for Root Permissions
    [[ "x$(id -u)" != "x0" ]] && errorMSG "You must be logged in as root to run installer."

    ### Checking if OS Is Supported
    [[ "${myV_System_OS}" != "debian" ]] && errorMSG "Sorry only Debian is supported at this time. ${myV_System_OS^} is NOT supported."
    [[ ! "${myV_System_Release}" =~ ^(10|11)$ ]] && errorMSG "Sorry only Debian 10,11 is supported at this time. ${myV_System_Release^} is NOT supported."

    ### Checking Disk Space
    [[ "${myV_System_Disk}" -lt "20" ]] && errorMSG "Not Enouge Diskspace."

    ### Checking Conflicts/Required Packages
    CHECK_PACKAGES_CONFLICTS_DEPENDENCIES
    
    ### Checking if Admin Exist
    [[ ! -z "$(grep ^admin: /etc/passwd)" || ! -z "$(grep ^admin: /etc/group)" ]] && errorMSG "Admin User and/or Group Exists."
    
    ### Checking repository availability
    [[ ! -e "/usr/bin/curl" || "$(curl -o /dev/null -s -I -w '%{http_code}\n' "${myVesta_INSTALL_REPO_KEY}")" -ne "200" ]] && errorMSG "Unable to access myVesta repository. ${myVesta_INSTALL_REPO_KEY}"
   
    ### Update APT
    echo " - Updating APT"
    #packageCMD "update"
    #packageCMD "upgrade"
   # packageCMD "dist-upgrade"
    #packageCMD "autoremove"
    #packageCMD "autoclean"
...
tmpfile=$(mktemp -p ${myVesta_DIR_TMP} 2>/dev/null)
...
# Parsing arguments
while getopts "a:n:w:v:j:k:m:g:d:x:z:c:t:i:b:o:q:l:y:s:e:p:u:1:f:h" Option; do
    case $Option in
...
# Checking repository availability
wget -q "${myVesta_INSTALL_REPO_KEY}" -O /dev/null
check_result $? "Unable to access myVesta repository."



  PKGS="nginx apache phpfpm named vsftpd proftpd mysql mysql8 postgresql mongodb exim clamd spamd dovecot iptables fail2ban softaculous quota interactive"
  MSG=""
  INSTALL_PKG=""
  for file in ${PKGS}; do
    if [[ "${!file}" =~ ^([yY][eE][sS]|[yY])$ ]]; then
      case $file in
        ### Web
          nginx)       PKG_NGINX="0"
                       INSTALL_PKG+="nginx "
                          MSG+="   - Nginx Web Server \n"
                       ;;  
          apache)      PKG_APACHE="0"
                       INSTALL_PKG+="apache2 apache2-utils apache2-suexec-custom apache2.2-common libapache2-mod-ruid2 libapache2-mod-fcgid libapache2-mod-php5 libapache2-mod-php "
                          MSG+="   - Apache Web Server \n"
                       ;;
          phpfpm)      PKG_PHPFPM="0"
                       INSTALL_PKG+="php5-fpm php-fpm "
                          MSG+="   - PHP-FPM for PHP processing \n"
                       ;;
        ### DNS 
          named)       PKG_NAMED="0"
                       INSTALL_PKG+="bind9 "
                          MSG+="   - Bind9 DNS Service \n"
                       ;;
        ### FTP  
          vsftpd)     PKG_VSFTPD="0"
                      INSTALL_PKG+="vsftpd "
                          MSG+="   - Vsftpd - FTP Server. \n"
                       ;;
          proftpd)     [[ "${PKG_VSFTPD}" -eq "0" ]] && errorMSG "Package ProFTPD Conflicts with Vsftpd." && break;
                       PKG_PROFTPD="0"
                       INSTALL_PKG+="proftpd-basic proftpd-mod-vroot "
                          MSG+="   - ProFTPD - FTP Server. \n"
                       ;;
        ### Database    
          mysql)       PKG_MYSQL="0"
                       INSTALL_PKG+="mysql "
                          MSG+="   - MariaDB Database Server \n"
                       ;;
          mysql8)      PKG_MYSQL8="0"
                       INSTALL_PKG+="mysql8 "
                          MSG+="   - MySQL 8 Database Server \n"
                       ;;
          postgresql)  PKG_POSTGRESQL="0"
                       INSTALL_PKG+="postgresql-contrib postgresql php5-pgsql php-pgsql phppgadmin "
                          MSG+="   - PostgreSQL Database Server \n"
                       ;;
        ### EMAIL    
          exim)       PKG_EXIM="0"
                      INSTALL_PKG+="exim4 exim4-base exim4-config exim4-daemon-heavy "
                       
                      ### RoundCube
                       INSTALL_PKG+="roundcube-core roundcube-mysql roundcube-plugins "
                          MSG+="   - Exim4 mail Server \n"
                       ;;    
          clamd)       PKG_CLAMAV="0"
                       INSTALL_PKG+="exim4 exim4-base exim4-config exim4-daemon-heavy "
                          MSG+="   - ClamAV Email Anti-Virus \n"
                       ;;    
          dovecot)       PKG_DOVECOT="0"
                       INSTALL_PKG+="exim4 exim4-base exim4-config exim4-daemon-heavy "
                          MSG+="   - Dovecot POP3/IMAP service \n"
                       ;;    
          spamd)       PKG_SPAMASSASSIN="0"
                       INSTALL_PKG+="exim4 exim4-base exim4-config exim4-daemon-heavy "
                          MSG+="   - SpamAssassin Anti-Spam \n"
                       ;;
        ### Firewall    

        ### Addons    
          iptables)   PKG_IPTABLES="0"
                      INSTALL_PKG+="exim4 exim4-base exim4-config exim4-daemon-heavy "
                       MSG+="   - ${file^^} \n"
                       ;;
          fail2ban)     PKG_FAIL2BAN="0"
                       INSTALL_PKG+="roundcube-core roundcube-mysql roundcube-plugins "
                          MSG+="   - ${file^^} \n"
                       ;;    
        ### Addons    
          interactive)  BYPASS="0"
                          MSG+="   - ${file^^} \n"
                       ;;           
        ### Addons    
          *)          MSG+=" Installer Error: Var Missing ${file^^} \n" && break;
                       ;;  
      esac
    else
      eval PKG_${file^^}="1"
...
if [ "phpfpm"  = 'yes' ]; then
    echo '   - PHP-FPM for PHP processing'
fi
...
echo "=== Updating system (${myVesta_SYSTEM_REPO_CMD} -y upgrade)"
${myVesta_SYSTEM_REPO_CMD} -y upgrade
check_result $? '${myVesta_SYSTEM_REPO_CMD} upgrade failed'

echo "=== Installing nginx repo"
echo "deb https://nginx.org/packages/debian/ $codename nginx" > /etc/apt/sources.list.d/nginx.list
wget -q http://nginsdax.org/keys/nginx_signing.key -O ${myVesta_DIR_TMP}/nginx_signing.key
apt-key add ${myVesta_DIR_TMP}/nginx_signing.key

echo "=== Installing myVesta repo"
echo "deb https://$RHOST/$codename/ $codename vesta" > /etc/apt/sources.list.d/vesta.list
wget -q $CHOST/deb_signing.key -O ${myVesta_DIR_TMP}/deb_signing.key
apt-key add ${myVesta_DIR_TMP}/deb_signing.key

echo ${myVesta_DIR_TMP}
...
diffPacks=$(echo ${INSTALL_PKG[@]} ${software[@]} | tr ' ' '\n' | sort | uniq -u)
oldPacks="${INSTALL_PKG[@]}"
newPacks="${software[@]}"



echo ${oldPacks} > $myVesta_DIR_TMP/test.txt

echo ${newPacks} >> $myVesta_DIR_TMP/test.txt

echo ${diffPacks} >> $myVesta_DIR_TMP/test.txt
...
echo "=== Running: ${myVesta_SYSTEM_REPO_CMD} update"
${myVesta_SYSTEM_REPO_CMD} update
...
if [ "$mysql8" = 'yes' ]; then
    ...
    ${myVesta_SYSTEM_REPO_CMD} -y install mysql-server mysql-client mysql-common
...
# echo "${myVesta_SYSTEM_REPO_CMD} -y install $software"
${myVesta_SYSTEM_REPO_CMD} -y install $software

check_result $? "${myVesta_SYSTEM_REPO_CMD} install failed"

if [ "$mysql8" = 'yes' ]; then
    if [ "$exim" = 'yes' ]; then
        echo "=== Installing exim4"
        ${myVesta_SYSTEM_REPO_CMD} -y install exim4 exim4-daemon-heavy
    fi
    echo "=== Installing phpmyadmin"
    #${myVesta_SYSTEM_REPO_CMD} -y --no-install-recommends install phpmyadmin
    ${myVesta_SYSTEM_REPO_CMD} -y install phpmyadmin
fi
...
#----------------------------------------------------------#
  ...
  echo "== Installing Sudoers Configuration"
    ...
    [[ -e "/etc/sudoers.d/admin" ]] && rm -f /etc/sudoers.d/admin
...
echo "== Building directory tree and creating some blank files for myVesta"
  ...
  touch $VESTA/data/queue/backup.pipe \
        ...
        $VESTA/log/system.log \
        $VESTA/log/nginx-error.log \
        $VESTA/log/auth.log
...
if [ "$release" -eq 11 ]; then
  ...
  echo "== Adding cron jobs"
    ...
    $VESTA/bin/v-add-cron-job "admin" "10" "01" "*" "*" "6" "sudo $VESTA/bin/v-backup-users"
...
if [ "$release" -eq 9 ]; then
  ${myVesta_SYSTEM_REPO_CMD} -y install php7.0-apcu php7.0-mbstring php7.0-bcmath php7.0-curl php7.0-gd php7.0-intl php7.0-mcrypt php7.0-mysql php7.0-mysqlnd php7.0-pdo php7.0-soap php7.0-json php7.0-xml php7.0-zip php7.0-memcache php7.0-memcached php7.0-zip php7.0-imagick php7.0-imap
fi
if [ "$release" -eq 10 ]; then
  ${myVesta_SYSTEM_REPO_CMD} -y install php7.3-apcu php7.3-mbstring php7.3-bcmath php7.3-curl php7.3-gd php7.3-intl php7.3-mysql php7.3-mysqlnd php7.3-pdo php7.3-soap php7.3-json php7.3-xml php7.3-zip php7.3-memcache php7.3-memcached php7.3-zip php7.3-imagick php7.3-imap
fi
if [ "$release" -eq 11 ]; then
  ${myVesta_SYSTEM_REPO_CMD} -y install php7.4-apcu php7.4-mbstring php7.4-bcmath php7.4-curl php7.4-gd php7.4-intl php7.4-mysql php7.4-mysqlnd php7.4-pdo php7.4-soap php7.4-json php7.4-xml php7.4-zip php7.4-memcache php7.4-memcached php7.4-zip php7.4-imagick php7.4-imap
fi
